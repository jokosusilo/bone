...
@section('content')
<form action="{{ route('cp.posts.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" id="title" class="form-control{{ $errors->has('title') ? ' is-invalid' : '' }}" name="title" autofocus="" value="{{ old('title') }}">
                        @include('cp.components.form-error', ['field' => 'title'])
                    </div>
                    <div class="form-group">
                        <label for="description">Deskripsi</label>
                        <textarea id="description" cols="30" rows="10" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" style="height: auto;" name="description">{{ old('description') }}</textarea>
